...
export const CandidateStudioPageX = () => {
    ...
    const { data: dates } = useElectionDeadlinesQuery(currentUser?.state, 'congress', currentUser?.district_l1_code);
    ...
    return (
        <div className="max-w-xl mx-auto flex flex-col gap-6 pb-20 w-full animate-in fade-in duration-200">
            {/* Display inline banner callout matrix */}
            {targetDeadline && (
                <div className="p-3 bg-rose-500/10 border border-rose-500/20 text-xs font-mono font-bold text-rose-400 rounded-xl mb-4 animate-pulse select-none">
                    ⚠️ POTENTIAL REGISTRATION DEADLINE: {targetDeadline}
                </div>
            )}
