const flattenComments = (comments) => {
    if (!comments) return [];
    const result = [];
    const recurse = (comment, depth) => {
        result.push({ ...comment, depth });
        if (comment.replies) {
            comment.replies.forEach(reply => recurse(reply, depth + 1));
        }
    };
    comments.forEach(comment => recurse(comment, 0));
    return result;
};

export const PostDetailsPageTest = () => {
    const { id } = useParams();
    ...
    // Find post in app state (top-level or comment/reply)
            ...
            if (p.comments) {
                ...
                if (found) {
                    post = {
                        ...
                        time: found.time,
                        likes: found.likes || 0,
                        commentsCount: found.replies ? found.replies.length : 0,
                        shares: found.boosts || 0,
                        image: found.image,
                        link: found.link,
                        contentWarning: found.contentWarning,
                        comments: found.replies || [],
                        liked: found.liked || false,
                        bookmarked: found.bookmarked || false,
                        parentPostId: p.id
                    };
                    break;
                }
...
second part, there are more parts, please wait
...
third part, there are more parts, please wait after this:
    ...
    return (
        <div className="max-w-[1280px] mx-auto flex flex-col lg:flex-row gap-12 pb-20">
            ...
            <div className="flex-1 flex flex-col gap-6 max-w-3xl">
                ...
                {/* 📝 Continuous Cascaded Stream (Mastodon style) */}
                <div className="flex flex-col border border-[#262626] bg-[#141414] rounded-[16px] overflow-hidden shadow-2xl">
                    {/* Featured Post Card */}
                    <article className="bg-[#181818] p-6 relative overflow-hidden border-b border-[#262626]">
                        {post?.comments && post?.comments.length > 0 && (
                            <div className="absolute top-[72px] bottom-0 w-[2px] bg-white/10 left-[48px] -translate-x-1/2 z-0"></div>
                        )}
                        ...
                        <div className="flex items-center justify-between mb-6 z-10 relative">
                            <div className="flex items-center gap-4">
                                <UserHoverCard author={post?.author}>
                                    <div className="w-12 h-12 rounded-xl overflow-hidden border border-white/10 hover:opacity-85 transition-opacity bg-[#1A1616]">
                                        {post?.author.avatar ? (
                        ...
                        {/* Post Content */}
                        <div className="mb-8 z-10 relative pl-16">
                            {post?.title && (
                        ...
                        {/* Interactions Row */}
                        <div className="flex items-center justify-between pt-6 border-t border-[#262626] pl-16 z-10 relative">
                            <div className="flex items-center gap-6">
                                <button
                                    onClick={() => toggleLike(post?.id)}
                                    className={`flex items-center gap-2 group transition-all bg-transparent border-none cursor-pointer ${post?.liked ? 'text-primary-container' : 'text-text-secondary hover:text-white'}`}
                                >
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 group transition-all bg-transparent border-none cursor-pointer ${post?.reblogged ? 'text-green-500' : 'text-text-secondary hover:text-white'
                                        }`}
                                    title="Boost Post"
                                >
                            ...
                            <button className="flex items-center gap-2 text-text-secondary hover:text-gold-muted transition-colors bg-transparent border-none cursor-pointer">
                                <span className="material-symbols-outlined">bookmark_add</span>
                            </button>
                        </div>
                    </article>

                    eighth, more parts coming

                    {/* Comment Form (inline in cascade) */}
                    {showCommentForm && (
                        <form onSubmit={handleCommentSubmit} className="p-6 bg-[#141414] border-b border-[#262626] space-y-4 animate-in fade-in slide-in-from-top-3 duration-200">
                    ...
                    13th part, more coming


                    {/* Comment Thread (inline in cascade) */}
                    {post?.comments.length === 0 ? (
                        <p className="text-text-secondary text-sm italic text-center py-10 bg-[#141414]">
                            No perspectives shared yet. Join the conversation!
                        </p>
                    ) : (() => {
                        const flatComments = flattenComments(post?.comments);
                        return flatComments.map((comment, idx) => {
                            const nextComment = flatComments[idx + 1];
                            const hasNextReply = nextComment && nextComment.depth > 0;
                            ...
                            if (comment.depth === 1) {
                                ...
                                for (let i = idx - 1; i >= 0; i--) {
                                    if (flatComments[i].depth === comment.depth - 1) {
                                        parentAuthorName = flatComments[i].author.name;
                                        break;
                                    }
                                }
                            }
...
const CommentCard = ({ comment, isReply = false, depth = 0, hasNextReply = false, parentAuthorName = null, onReplyClick }) => {
    const navigate = useNavigate();
    const [liked, setLiked] = useState(false);
    const [likesCount, setLikesCount] = useState(comment.likes || 0);

    const handleCardClick = (e) => {
        if (e.target.closest('button') || e.target.closest('a') || e.target.closest('input') || e.target.closest('.user-hover-trigger') || e.target.closest('span.cursor-pointer')) {
            return;
        }
    ...
    return (
        <div
            ...
            className={`flex flex-col relative p-6 border-b border-[#262626] last:border-b-0 cursor-pointer hover:bg-white/[0.015] transition-colors ${isReply ? 'pl-20 bg-[#141414]' : 'bg-[#181818]'}`}
        >
            {/* Downward line from root avatar */}
            {depth === 0 && hasNextReply && (
                <div className="absolute top-[72px] bottom-0 w-[2px] bg-white/10 left-[48px] -translate-x-1/2 z-0"></div>
            )}

            {/* L-branch for direct replies (depth === 1) */}
            {depth === 1 && (
                <div className="absolute top-0 left-[48px] w-14 h-12 border-l-2 border-b-2 border-white/10 rounded-bl-xl pointer-events-none z-0"></div>
            )}

            {/* Straight line above avatar for sub-replies (depth > 1) */}
            {depth > 1 && (
                <div className="absolute top-0 h-[30px] w-[2px] bg-white/10 left-[104px] -translate-x-1/2 z-0"></div>
            )}

            {/* Downward line from reply avatar */}
            {depth > 0 && hasNextReply && (
                <div className="absolute top-[72px] bottom-0 w-[2px] bg-white/10 left-[104px] -translate-x-1/2 z-0"></div>
            )}
            ...
            <div className="flex gap-4 z-10">
                <div className="flex flex-col items-center flex-shrink-0">
                    <UserHoverCard author={comment.author}>
                        <div className="w-12 h-12 rounded-xl overflow-hidden flex-shrink-0 border border-white/10 hover:opacity-85 transition-opacity bg-[#1A1616]">
                ...
                <div className="flex-1 min-w-0">
                    {isReply && parentAuthorName && (
                        <div className="flex items-center gap-1 text-[13px] text-text-secondary/60 mb-1 leading-none select-none">
                            ...
                            <button
                                ...
                                className="font-bold text-primary-container hover:underline cursor-pointer bg-transparent border-none p-0 text-[13px] inline-flex items-center gap-0.5"
                            >
